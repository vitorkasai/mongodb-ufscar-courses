...
def cadastrar_novo_semestre():
    ...
    while numero_semestre not in ["1", "2"]:
        print("Número do semestre inválido. Tente novamente.")
        numero_semestre = input("Digite o número do semestre (ex: 1 ou 2): ").strip()
    ...
    if resultado.acknowledged:
        print(f"Semestre {ano}/{numero_semestre} cadastrado com sucesso!")
        return semestre
    else:
        print("Erro ao cadastrar semestre.")
        return {}
...
def buscar_semestres_por_status():
    ...
    if semestres_lista:
        ...
        print(f"Nenhum semestre encontrado com status '{status}'.")
        return []
...
def atualizar_semestre(id_semestre):
    ...
    if novos_dados:
        ...
        print("Nenhum dado novo foi fornecido para atualização.")
        return {}

def deletar_semestre(id_semestre):
    """Remove um semestre."""
    semestre_encontrado = find_semestre_by_id(id_semestre)
    print(f"Você está prestes a excluir o semestre '{semestre_encontrado['ano']}/{semestre_encontrado['numero_semestre']}'.")
